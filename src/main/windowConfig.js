...
// ⭐ 중요: imageManager는 main 프로세스에서 직접 로드하지 않습니다.
// IPC를 통해 renderer 프로세스에서 이미지를 관리해야 합니다.
// 이 부분은 windowConfig.js의 역할이 아니므로 제거해야 합니다.
// const imageManager = require(pathJoin.getModulePath() + '/imageManager');

// config.json 파일 경로를 앱의 사용자 데이터 폴더에 저장하는 것이 좋습니다.
// Electron의 app 모듈은 main 프로세스에서만 사용 가능하므로,
// Electron의 app.getPath('userData')를 사용하려면 이 파일을 main 프로세스에서 로드하거나
// 경로를 main 프로세스에서 받아와야 합니다.
// 여기서는 간단하게 shared/config.json을 사용하던 기존 방식을 유지합니다.
...
async function getWindowConfig() {
    ...
    if (mode === "image") {
        // ⭐ 중요: getInitialImagePath는 renderer 프로세스에서 imageManager가 처리해야 할 부분입니다.
        // main 프로세스에서 이미지 파일 경로를 직접 얻으려면, renderer를 거치지 않고 직접 이미지 폴더를 읽어야 합니다.
        // 여기서는 임시로 첫 번째 이미지를 가져오는 로직을 직접 구현하거나,
        // 이 로직을 main 프로세스의 다른 적절한 위치로 옮겨야 합니다.
        // 현재 코드에서는 imageManager가 필요 없는 방식으로 변경합니다.
...
module.exports = {
    getWindowConfig,
    saveWindowConfig // ⭐ saveWindowConfig를 내보냅니다! ⭐
};